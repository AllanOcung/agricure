// --- Page-Specific Initializer for Recommendations Page ---
function initializeRecommendationsScript() {
  ...
  const partialUrl = window.appConfig.recommendationsPartialUrl;
...
// --- Page-Specific Initializer for Dashboard Page ---
...
function initializeIoTScript() {
  ...
  function initializeCharts() {
    ...
    console.log("IoT: iotDataDiv found:", iotDataDiv);
  ...
  function changeTimeRange(range) {
    ...
    fetch(`/iot/api/historical/?range=${range}`)
      .then((response) => response.json())
      .then((data) => {
        // Update historical data in the hidden div
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.type === "childList" && mutation.target === pageContainer) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to run after new page content is loaded via AJAX
  function onPageLoad() {
    console.log("App: onPageLoad called");
    lucide.createIcons();
    initializeRecommendationsScript();
    initializeDashboardScript();
    console.log("App: About to call initializeIoTScript");
    initializeIoTScript();
    console.log("App: initializeIoTScript called");
  ...
  const fetchAndLoadPage = (url, pushState = true) => {
    ...
    fetch(url, { headers: { "X-Requested-With": "XMLHttpRequest" } })
      ...
      .then((data) => {
        ...
        onPageLoad(); // Re-run all initializers for new content
      })
      .catch((error) => {
  ...
  initializeGlobalScripts(); // For static elements like the navbar
  onPageLoad(); // For the initial page content
});
